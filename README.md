...
1. Load the raw vibration signal files.
2. Segment each continuous signal into fixed-length samples.
3. Assign the corresponding bearing condition label.
4. Split the samples into training, validation, and test sets.
5. Train and evaluate a fault diagnosis model.

Example pseudocode:

```python
# Example only: adapt paths and file formats to your released dataset
signals = load_signal_file("data/inner_race_fault/sample_001.csv")
segments = segment_signal(signals, window_size=2048, step_size=1024)
labels = [1] * len(segments)
```

## Citation

If you use this dataset in academic work, please cite the related paper or project. Replace the placeholder below with the formal citation information.

```bibtex
@misc{high_speed_bearing_fault_dataset,
  title  = {High-Speed Bearing Fault Diagnosis Dataset},
  author = {Your Name or Research Group},
  year   = {Year},
  note   = {Laboratory high-speed bearing fault simulation test dataset}
}
```

## License

Please specify the license before releasing the dataset. Common choices include:
